        ...
        _renderTemplate:function (data) {
            ...
            for (var i = 0, tmpList; tmpList = data[i++];) {
                ...
                for (var j = 0, tmpObj; tmpObj = tmpList[j++];) {
                    ...
                    s.push('<input type="radio" id="radio-' + i + '-' + j + '" name="musicId" class="m-l" onclick="music.doselect(' + (me.size * (i-1) + (j-1)) + ')"/>');
                    ...
                    s.push('<span class="m-try" onclick="music.doselect(' + (me.size * (i-1) + (j-1)) + ');music.listenTest(this)"></span>');
                    s.push('</label>');
                }
                s.push('</div>');
                s.push('</div>');
